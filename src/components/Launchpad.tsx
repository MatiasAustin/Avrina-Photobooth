      ...
      <main className="relative z-10 max-w-7xl mx-auto p-8 pt-12">
          {filteredEvents.length === 0 ? (
            <div className="py-32 text-center space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
               <div className="w-24 h-24 bg-[var(--color-pawtobooth-light)] border border-black/5 rounded-[2.5rem] flex items-center justify-center mx-auto text-[var(--color-pawtobooth-dark)]/40">
                  <Power className="w-12 h-12" />
               </div>
               <div className="space-y-2">
                  <h2 className="text-3xl font-bold uppercase tracking-tight italic">No Active Booths</h2>
                  <p className="text-[var(--color-pawtobooth-dark)]/60 max-w-xs mx-auto text-sm">Please activate your booths in the main dashboard to see them here.</p>
               </div>
               <Link to="/dashboard" className="inline-block px-12 py-4 bg-[#3E6B43] text-white font-black uppercase text-xs tracking-[0.2em] rounded-2xl hover:scale-105 transition-transform shadow-md">
                 Go to Dashboard
               </Link>
            </div>
         ) : (
